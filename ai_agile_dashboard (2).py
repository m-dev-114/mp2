...
"""
    ...
    with tabs[0]:
        ...
        st.caption("Adjust team capacity and see how it affects the project health score in real time.")

        if 'sprint' in models and 'workload' in models and 'burnout' in models:
            wi_col1, wi_col2 = st.columns([1, 1])
            with wi_col1:
                st.markdown("**⚙️ Simulation Controls**")
                assignee_col_wi = 'Assignee' if 'Assignee' in df.columns else None
                assignees_wi = sorted(df[assignee_col_wi].unique()) if assignee_col_wi else []

                selected_assignee = st.selectbox("Select team member to adjust", ["All team"] + assignees_wi, key="wi_assignee")
                sp_adjustment = st.slider("Story Point load adjustment (%)", -50, 50, 0, 5, key="wi_sp",
                                          help="Negative = reduce load, Positive = increase load")
                workload_adjustment = st.slider("Workload % adjustment", -60, 60, 0, 5, key="wi_wl",
                                                help="Simulate adding/removing tasks")
                unblock_stories = st.slider("Reduce blocked stories by", 0, 5, 0, key="wi_bs",
                                            help="Simulate unblocking sprint blockers")

            with wi_col2:
                st.markdown("**📊 Simulated Outcome**")
                df_sim = df.copy()
                df_sim['Success_Label']     = (df_sim['Success_Label']     > 0.5).astype(int)
                df_sim['Expected_Overload'] = (df_sim['Expected_Overload'] > 0.5).astype(int)
                df_sim['Risk_Flag']         = (df_sim['Risk_Flag']         > 0.3).astype(int)

                # Apply adjustments
                mask = (df_sim[assignee_col_wi] == selected_assignee) if (selected_assignee != "All team" and assignee_col_wi) else pd.Series([True]*len(df_sim), index=df_sim.index)
                if 'Current_Assigned_SP' in df_sim.columns:
                    df_sim.loc[mask, 'Current_Assigned_SP'] *= (1 + sp_adjustment / 100)
                if 'Current_Workload_Percent' in df_sim.columns:
                    df_sim.loc[mask, 'Current_Workload_Percent'] += workload_adjustment
                if 'Blocked_Stories' in df_sim.columns:
                    df_sim.loc[mask, 'Blocked_Stories'] = (df_sim.loc[mask, 'Blocked_Stories'] - unblock_stories).clip(0)

                # Re-run models
                sim_findings = []
                try:
                    m1 = models['sprint']['model']
                    X1s = df_sim[models['sprint']['features']]
                    p1 = m1.predict(X1s)
                    prob1 = m1.predict_proba(X1s)[:,1]
                    at_risk_s = int((p1==0).sum())
                    pct_s = at_risk_s / len(p1)
                    sev_s = 'critical' if pct_s > 0.4 else 'warning' if pct_s > 0.2 else 'success'
                    sim_findings.append({'severity': sev_s, 'title': f"Sprint at risk: {at_risk_s} ({pct_s:.0%})"})
                except: pass

                try:
                    m2 = models['workload']['model']
                    X2s = df_sim[models['workload']['features']]
                    p2 = m2.predict(X2s)
                    prob2 = m2.predict_proba(X2s)[:,1]
                    ovl_s = int(p2.sum())
                    pct_2 = ovl_s / len(p2)
                    avg_wl = df_sim.loc[p2==1,'Current_Workload_Percent'].mean() if ovl_s > 0 else 0
                    sev_2 = 'critical' if pct_2 > 0.3 else 'warning' if pct_2 > 0.15 else 'success'
                    sim_findings.append({'severity': sev_2, 'title': f"Overloaded: {ovl_s} ({pct_2:.0%}), avg {avg_wl:.0f}%"})
                except: pass

                try:
                    m4 = models['burnout']['model']
                    X4s = df_sim[models['burnout']['features']]
                    p4 = m4.predict(X4s)
                    burn_s = int(p4.sum())
                    pct_4 = burn_s / len(p4)
                    sev_4 = 'critical' if pct_4 > 0.4 else 'warning' if pct_4 > 0.25 else 'success'
                    sim_findings.append({'severity': sev_4, 'title': f"Burnout risk: {burn_s} ({pct_4:.0%})"})
                except: pass

                sim_score = 100
                for sf in sim_findings:
                    if sf['severity'] == 'critical': sim_score -= 25
                    elif sf['severity'] == 'warning': sim_score -= 10
                sim_score = max(0, min(100, sim_score))

                delta_score = sim_score - score
                s_color = "#06d6a0" if sim_score >= 75 else ("#ffd166" if sim_score >= 50 else "#ff4d6d")
                s_label = "🟢 Healthy" if sim_score >= 75 else ("🟡 Needs Attention" if sim_score >= 50 else "🔴 At Risk")
                delta_str = f"{'▲' if delta_score >= 0 else '▼'} {abs(delta_score)} pts vs baseline"
                delta_color = "#06d6a0" if delta_score > 0 else ("#aaa" if delta_score == 0 else "#ff4d6d")

                st.markdown(f"""
                <div style='text-align:center;padding:1rem;background:#1a1a2e;border-radius:12px;border:1px solid #333355;'>
                    <div style='font-size:3rem;font-weight:900;color:{s_color};'>{sim_score}</div>
                    <div style='font-size:0.75rem;color:#aaa;'>/100 — {s_label}</div>
                    <div style='font-size:0.9rem;color:{delta_color};margin-top:6px;font-weight:700;'>{delta_str}</div>
                </div>
                """, unsafe_allow_html=True)

                st.markdown("<br>", unsafe_allow_html=True)
                sev_map = {'critical':'🔴','warning':'🟡','success':'✅'}
                for sf in sim_findings:
                    st.markdown(f"{sev_map.get(sf['severity'],'ℹ️')} {sf['title']}")
        else:
            st.info("ℹ️ Models not yet trained. Upload a dataset to enable the simulator.")
