...
def create_issue(body:IssueCreate):
    nid = f"AGI-{len(_issues)+1:04d}"
    eh  = round(body.story_points * float(np.random.uniform(1.5, 2.5)), 1)

    # ── Build raw issue record ────────────────────────────────────────────
    row = {
        "issue_id":               nid,
        "sprint_id":              body.sprint_id,
        "sprint_name":            next((s["sprint_name"] for s in _sprints
                                        if s["sprint_id"]==body.sprint_id), "Sprint 8"),
        "summary":                body.summary,
        "issue_type":             body.issue_type,
        "priority":               body.priority,
        "status":                 "To Do",
        "assignee":               body.assignee,
        "employee_id":            next((e["employee_id"] for e in _employees
                                        if e["name"]==body.assignee), "EMP-000"),
        "team":                   next((e["team"] for e in _employees
                                        if e["name"]==body.assignee), "Unknown"),
        "story_points":           body.story_points,
        "original_estimate_hours":eh,
        "time_spent_hours":       0.0,
        "resolution_time_hours":  eh,
        "labels":                 body.labels,
    ...
    # ── Derive ML row so it immediately appears in /api/dataset/ml ────────
    spr     = next((s for s in _sprints if s["sprint_id"]==body.sprint_id), {})
    emp     = next((e for e in _employees if e["name"]==body.assignee),     {})
    pln     = float(spr.get("planned_sp", 80))
    hv      = float(spr.get("historical_velocity", 70))
    blk     = float(spr.get("blocked_stories", 2))
    sc      = float(spr.get("scope_change", 0))
    dr      = float(spr.get("days_remaining", 3))
    cmp     = float(spr.get("completed_sp", 55))
    pct     = float(spr.get("percent_done", 65))
    bv      = float(emp.get("base_velocity", 40))
    exp_yrs = float(emp.get("experience_yrs", 3))
    hist_sp = bv
    asgn_sp = float(body.story_points) * 4.0
    wl      = float(np.clip(80 + (asgn_sp - hist_sp) * 1.1, 40, 200))
    hpt     = 2; co = 1; rdr = 5.0
    sp      = float(body.story_points)
    ...
    ml_row = {
        ...
        "Blocked_Stories":               blk,
        "Scope_Change":                  sc,
        "Success_Label":                 int(pct >= 60 and blk < 5),
        "Sprint_Number":                 int(spr.get("sprint_number", 8)),
        "Planned_Story_Points_Resource": round(hist_sp * 0.9, 1),
        "Current_Assigned_SP":           round(asgn_sp, 1),
        "Historical_Avg_SP":             round(hist_sp, 1),
        "Remaining_Days_Resource":       rdr,
        ...
        "Original_Estimate_Resource":    eh,
        "Story_Points_Resource":         sp,
        "Assignee_Resource":             body.assignee,
        "Assignee":                      body.assignee,
        "Issue_ID":                      nid,
        "Status":                        "To Do",
        "Team":                          row["team"],
    ...
    return {
        ...
        "issue":     row,
        "ml_row_added": True,
        "ml_dataset_size": len(_ml_dataset),
    }
...
def transition(issue_id:str,body:TransitionCreate):
    ...
    # Sync status in ML dataset too
    ml_row=next((r for r in _ml_dataset if r.get("Issue_ID")==issue_id),None)
    if ml_row:
        ml_row["Status"]=body.status
        if body.status=="Done":
            ml_row["Success_Label"]=1
    return {"transitioned":True,"issue_id":issue_id,"from":old,"to":body.status,
            "ml_dataset_synced": ml_row is not None}